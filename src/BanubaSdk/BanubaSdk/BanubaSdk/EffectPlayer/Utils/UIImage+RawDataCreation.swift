//
//  UIImage+RawDataCreation.swift
//  BanubaSdk
//
//  Created by Andrey Bondar on 12/11/18.
...
public extension UIImage
{
    ...
    convenience init?(rgbaDataNoCopy: NSData, width: Int, height: Int) {
    ...
    func makeBgraPixelBuffer() -> CVPixelBuffer? {
        ...
        let cgContext = CGContext(data: pixelBufferData, width: width, height: height, bitsPerComponent: 8, bytesPerRow: bytesPerRow,
...
fileprivate extension CVPixelBuffer {
    ...
    func makeImageBufferInfo(planeIndex: Int? = nil) -> vImage_Buffer {
        if let index = planeIndex {
            return vImage_Buffer(data: CVPixelBufferGetBaseAddressOfPlane(self, index),
        ...
        return vImage_Buffer(data: CVPixelBufferGetBaseAddress(self),
