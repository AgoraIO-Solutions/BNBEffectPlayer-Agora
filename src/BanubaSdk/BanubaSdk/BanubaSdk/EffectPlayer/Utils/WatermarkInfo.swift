...
public struct WatermarkInfo {
    ...
    public init(image: UIImage, corner: WatermarkCornerPosition, offset: CGPoint, targetWidth: CGFloat) {
    ...
    public init(image: UIImage, corner: WatermarkCornerPosition, offset: CGPoint, targetNormalizedWidth: CGFloat) {
        isCornerPositioned = true
    ...
    public init(image: UIImage, normalizedPosition: CGPoint, targetWidth: CGFloat) {
    ...
    public init(image: UIImage, normalizedPosition: CGPoint, targetNormalizedWidth: CGFloat) {
        isCornerPositioned = false
        isNormalizedSizeUsed = true
    ...
    func drawSettingsWithBoundsSize(_ boundsSize: CGSize, outputSettings: OutputSettings) -> WatermarkDrawSettings {
        ...
        if isCornerPositioned {
            let rotatedOffset = outputSettings.deviceOrientation.isLandscape ? CGPoint(x: cornerOffset.y, y: cornerOffset.x) : cornerOffset
            let rotatedSize = outputSettings.deviceOrientation.isLandscape ? CGSize(width: size.height, height: size.width) : size
...
fileprivate extension WatermarkCornerPosition {
    ...
    func adjustWithDeviceOrientation(_ orientation: UIDeviceOrientation) -> WatermarkCornerPosition {
        let sides = WatermarkCornerPosition.allCases
